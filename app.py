...
# 1. CONFIGURAÇÃO DA PÁGINA (Layout Wide e Título)
st.set_page_config(page_title="Zion - Dashboard Executivo", layout="wide")

# CSS para tornar a interface mais compacta
st.markdown("""
    <style>
    .block-container {padding-top: 1rem; padding-bottom: 1rem;}
    h3 {margin-top: 0rem; margin-bottom: 0.5rem; font-size: 1.2rem;}
...
try:
    ...
    # --- MAPEAMENTO FORECAST (Intervalo U2 até AA8) ---
    # Coluna U=20 até AA=26. Linhas de dados 0 a 7.
    bloco_forecast = df_full.iloc[0:7, 20:27].astype(str).apply(lambda x: x.str.strip().str.upper())
    lista_forecast = pd.unique(bloco_forecast.values.ravel())
    lista_forecast = [nome for nome in lista_forecast if nome not in ['NAN', '', 'NONE', '0']]
    
    # --- MAPEAMENTO OPERACIONAL (B até O) ---
    df_op = df_full.iloc[0:50, 0:15].copy()
    col_emp = df_op.columns[2] # Coluna C (Empurrador)
    df_op = df_op.dropna(subset=[col_emp])
    
    # Padronização para cálculo e comparação
    ...
    # --- TABELA 1: RESUMO CONSOLIDADO (COMPACTO) ---
    ...
    if alertas:
        cols_alerta = st.columns(len(alertas) if len(alertas) < 4 else 4)
        for i, msg in enumerate(alertas):
            cols_alerta[i % 4].warning(msg)

    st.markdown("---")

    # --- TABELA 2: REALIZADOS (ELEGANTE E COMPACTO) ---
    ...
    if not df_real.empty:
        # Selecionando colunas e ajustando largura
