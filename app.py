...
def carregar_dados(nome_aba):
    ...
    # Lemos como string pura para não deixar o Pandas "sumir" com os dados
    df = pd.read_csv(url, dtype=str).fillna("")
    return df

def renderizar_rancho(df):
    try:
        # MAPEAMENTO POR ÍNDICE (Garantindo que pegue as colunas certas da imagem)
        # B=1 (Status), G=6 (SC), K=10 (Empurrador), R=17 (Data), T=19 (Local)
        ...
        # FILTRO ÚNICO: Tudo que for PROGRAMADO aparece. Sem travas de data.
        df_prog = df[df[col_status].astype(str).str.upper().str.contains('PROGR', na=False)].copy()

        # Ordenamos apenas para o dia 14 vir antes do 21 e 23
        df_prog['ORDEM_AUX'] = pd.to_datetime(df_prog[col_entrega], dayfirst=True, errors='coerce')
        df_prog = df_prog.sort_values(by='ORDEM_AUX', ascending=True)

        st.markdown("### 📅 RANCHOS PROGRAMADOS")
        
        if not df_prog.empty:
            # Exibe exatamente o que você tem na planilha (inclusive Cumaru 23/03 e Aroeira 21/03)
            st.dataframe(
            ...
            st.warning("Não foram encontrados dados 'PROGRAMADOS' na aba Rancho.")
...
try:
    if aba == "Combustível (ODM)":
        # RESTAURANDO LÓGICA ORIGINAL DO COMBUSTÍVEL
