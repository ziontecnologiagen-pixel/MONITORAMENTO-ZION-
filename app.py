import streamlit as st
import pandas as pd
from urllib.parse import quote

# 1. CONFIGURAÇÃO INICIAL
st.set_page_config(page_title="Zion - Gestão Integrada", layout="wide")

SHEET_ID = "1izHisQGFCLdqQ7d2OSGkAM7gDJrIsLxW9FY741lJ_Ao"

@st.cache_data(ttl=2)
def carregar_dados(nome_aba):
    try:
        url = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/gviz/tq?tqx=out:csv&sheet={quote(nome_aba)}"
        # Lemos tudo como string para evitar que números como os da SC sumam
        df = pd.read_csv(url, dtype=str).fillna("")
        return df
    except:
        st.error(f"Erro ao conectar com a aba {nome_aba}")
        return pd.DataFrame()

def destacar_meio_rancho(row, col_desc):
    """Aplica o fundo laranja apenas na célula de descrição"""
    estilo = [''] * len(row)
    if "MEIO RANCHO" in str(row[col_desc]).upper():
        idx = row.index.get_loc(col_desc)
        estilo[idx] = 'background-color: #FFA500; color: black; font-weight: bold'
    return estilo

def exibir_rancho():
    df = carregar_dados("RANCHO")
    if df.empty: return

    try:
        # ÍNDICES TRAVADOS (A=0, B=1, G=6, J=9, K=10, L=11, N=13, P=15, S=18)
        col_prox_ped = df.columns[0]   # A
        col_status   = df.columns[1]   # B
        col_sc       = df.columns[6]   # G (Onde está o 20864, 20866)
        col_local    = df.columns[9]   # J
        col_emp      = df.columns[10]  # K
        col_comp     = df.columns[11]  # L
        col_entrega  = df.columns[13]  # N
        col_prox_ent = df.columns[15]  # P
        col_desc     = df.columns[18]  # S

        hoje = pd.Timestamp(2026, 3, 15)

        # --- TABELA 1: PROGRAMADOS ---
        df_prog = df[df[col_status].astype(str).str.upper().str.contains('PROGR', na=False)].copy()
        df_prog['DT_OBJ'] = pd.to_datetime(df_prog[col_entrega], dayfirst=True, errors='coerce')
        df_futuro = df_prog[df_prog['DT_OBJ'] >= hoje].sort_values(by='DT_OBJ')

        st.markdown("### 📅 RANCHOS PROGRAMADOS")
        if not df_futuro.empty:
            st.dataframe(
                df_futuro[[col_emp, col_sc, col_entrega, col_desc]], 
                use_container_width=True, hide_index=True
            )

        st.divider()

        # --- TABELA 2: ENTREGUES (REALIZADOS) ---
        # Filtro por Competência "03" e Status "REALIZADO"
        df_real = df[
            (df[col_status].astype(str).str.upper() == 'REALIZADO') & 
            (df[col_comp].astype(str).str.contains('03', na=False))
        ].copy()

        st.markdown("### ✅ Rancho Entregues no Mês Corrente")
        if not df_real.empty:
            cols_entregues = [col_emp, col_sc, col_local, col_entrega, col_prox_ent, col_desc, col_prox_ped]
            
            # Resetamos o índice para garantir que o estilo não quebre a coluna SC
            df_display = df_real[cols_entregues].reset_index(drop=True)
            
            st.dataframe(
                df_display.style.apply(destacar_meio_rancho, col_desc=col_desc, axis=1),
                use_container_width=True, hide_index=True,
                column_config={col_sc: "SC", col_local: "SETOR/LOCAL", col_prox_ped: "PRÓXIMO PEDIDO"}
            )
        else:
            st.info("Nenhuma entrega realizada encontrada para Março/2026.")

    except Exception as e:
        st.error(f"Erro na estrutura do Rancho: {e}")

def exibir_combustivel():
    # Voltando com a lógica que funcionava para o ODM
    df_odm = carregar_dados("ODM MARÇO")
    if not df_odm.empty:
        st.markdown("### ⛽ Gestão de Combustível (ODM)")
        st.dataframe(df_odm, use_container_width=True, hide_index=True)
    else:
        st.warning("Aba 'ODM MARÇO' não encontrada ou vazia.")

# --- NAVEGAÇÃO PRINCIPAL ---
st.title("🚢 Sistema de Gestão Zion")
aba_selecionada = st.radio("Selecione:", ["Combustível (ODM)", "Rancho"], horizontal=True)

if aba_selecionada == "Rancho":
    exibir_rancho()
else:
    exibir_combustivel()
